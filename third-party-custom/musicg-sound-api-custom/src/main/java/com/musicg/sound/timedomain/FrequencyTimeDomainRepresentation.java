	...
	public void buildSpectrogram(){
		...
		if (rebuildSpectrogram){
			...
			if (absoluteSpectrogram.length>0){
				...
				int numFreqs=absoluteSpectrogram[0].length;
				unitFrequency=(double)wave.getSampleRate()/2/numFreqs;	// frequency could be caught within the half of nSamples according to Nyquist theory
					
				// normalization of absoultSpectrogram
				spectrogram=new double[numFrames][numFreqs];
