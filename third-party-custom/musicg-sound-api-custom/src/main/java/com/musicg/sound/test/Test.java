...
//        // change the amplitude representation
        ...
        float timeStep          =
                ( float ) ( nAmplitudes.length / ampRp.getWave().getSampleRate() )
                / ( 32000 );

        System.out.println( "*** Setting time step: " + timeStep + "( with max = " + 65500 + " )" );
        ampRp.setTimeStep( timeStep );

        render.renderWaveform( ampRp, strProjectOutput + "original.jpg" );
